...
import (
	...
	"chaincode/orchestrator/objective"
	"encoding/json"
	"strings"
	"sync"
...
func (db *LedgerDB) GetAssetType(key string) (asset.AssetType, error) {
	asset := struct {
...
func (db *LedgerDB) GetAlgo(key string) (Algo, error) {
	...
	if algo.AssetType != asset.AlgoType {
...
func (db *LedgerDB) GetCompositeAlgo(key string) (CompositeAlgo, error) {
	...
	if algo.AssetType != asset.CompositeAlgoType {
		return algo, errors.NotFound("algo %s not found", key)
	}
	return algo, nil
}
...
func (db *LedgerDB) GetAggregateAlgo(key string) (AggregateAlgo, error) {
	...
	if algo.AssetType != asset.AggregateAlgoType {
		return algo, errors.NotFound("algo %s not found", key)
	}
	return algo, nil
}
...
func (db *LedgerDB) GetObjective(key string) (objective.Objective, error) {
	objective := objective.Objective{}
	if err := db.Get(key, &objective); err != nil {
		return objective, err
	}
	if objective.AssetType != asset.ObjectiveType {
...
func (db *LedgerDB) GetDataManager(key string) (DataManager, error) {
	...
	if dataManager.AssetType != asset.DataManagerType {
...
func (db *LedgerDB) GetDataSample(key string) (DataSample, error) {
	...
	if dataSample.AssetType != asset.DataSampleType {
...
func (db *LedgerDB) GetTraintuple(key string) (Traintuple, error) {
	...
	if traintuple.AssetType != asset.TraintupleType {
...
func (db *LedgerDB) GetCompositeTraintuple(key string) (CompositeTraintuple, error) {
	...
	if traintuple.AssetType != asset.CompositeTraintupleType {
...
func (db *LedgerDB) GetAggregatetuple(key string) (Aggregatetuple, error) {
	...
	if aggregatetuple.AssetType != asset.AggregatetupleType {
...
func (db *LedgerDB) GetComputePlan(key string) (ComputePlan, error) {
	...
	if computePlan.AssetType != asset.ComputePlanType {
...
func (db *LedgerDB) GetOutModelKeyChecksumAddress(tupleKey string, allowedAssetTypes []asset.AssetType) (*KeyChecksumAddress, error) {
	for _, assetType := range allowedAssetTypes {
		switch assetType {
		case asset.CompositeTraintupleType:
			tuple, err := db.GetCompositeTraintuple(tupleKey)
			if err != nil {
				continue
			}
			return tuple.OutTrunkModel.OutModel, nil
		case asset.TraintupleType:
			tuple, err := db.GetTraintuple(tupleKey)
			if err == nil {
				return tuple.OutModel, nil
			}
		case asset.AggregatetupleType:
...
func (db *LedgerDB) GetTesttuple(key string) (Testtuple, error) {
	...
	if testtuple.AssetType != asset.TesttupleType {
