...
func (traintuple *CompositeTraintuple) Save(db *LedgerDB, traintupleKey string) error {
	...
	if err := db.CreateIndex("tuple~inModel~key", []string{"compositeTraintuple", traintuple.InHeadModel, traintupleKey}); err != nil {
		return err
	}
	if err := db.CreateIndex("tuple~inModel~key", []string{"compositeTraintuple", traintuple.InTrunkModel, traintupleKey}); err != nil {
		return err
	}
