...
var recursiveLogSuccessFailCompositeTests = []struct {
	head  AssetType
	trunk AssetType
	child AssetType
}{
...
func TestRecursiveLogCompositeSuccessFail(t *testing.T) {
	for _, tt := range recursiveLogSuccessFailCompositeTests {
		for _, status := range []string{"successHead", "successTrunk", "successBoth", "failHead", "failTrunk"} {
			testName := fmt.Sprintf("TestRecursiveLogComposite_%s_%sHead_%sTrunk_%sChild", status, tt.head, tt.trunk, tt.child)
			t.Run(testName, func(t *testing.T) {
				scc := new(SubstraChaincode)
				mockStub := NewMockStubWithRegisterNode("substra", scc)
				registerItem(t, *mockStub, "compositeAlgo")
